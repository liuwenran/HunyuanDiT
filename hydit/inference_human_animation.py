...
from .diffusion.pipeline_human_animation import StableDiffusionPipeline
from .modules.models import HunYuanDiT, HUNYUAN_DIT_CONFIG, HUNYUAN_DIT_MODELS
...
from hydit.train_human_animation_stage1 import Net
...
class ResolutionGroup:
    def __init__(self):
        self.data = [
            ...
            Resolution(512, 896),  # human animation size
            Resolution(896, 512),  # human animation size
        ]
...
class End2End(object):
    def __init__(self, args, models_root_path):
        ...
        if self.infer_mode in ['fa', 'torch']:
            ...
            self.model_reference = model_reference.half().to(self.device)
            self.pose_guider = pose_guider.half().to(self.device)
