...
from ..modules.mutual_self_attention import ReferenceAttentionControl
...
class StableDiffusionPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def __init__(
            ...
            reference_unet: Union[HunYuanDiT, UNet2DConditionModel],
            pose_guider: Union[HunYuanDiT, UNet2DConditionModel],
            scheduler: KarrasDiffusionSchedulers,
            safety_checker: StableDiffusionSafetyChecker,
            feature_extractor: CLIPImageProcessor,
            image_encoder: ReferenceAttentionControl = None,
    ...
    def __call__(
        ...
        hidden_size = self.unet.hidden_size
        num_tokens = self.unet.x_embedder.num_patches
        reference_control_writer = ReferenceAttentionControl(
            self.reference_unet,
            do_classifier_free_guidance=do_classifier_free_guidance,
            mode="write",
            batch_size=batch_size * 2 if do_classifier_free_guidance else batch_size,
            fusion_blocks="full",
        ...
        ref_image_tensor = self.image_processor.preprocess(
            ref_image, height=height, width=width
        )  # (bs, c, width, height)
        ref_image_tensor = ref_image_tensor.to(
            dtype=self.vae.dtype, device=self.vae.device
        )
        ref_image_latents = self.vae.encode(ref_image_tensor).latent_dist.sample().mul_(vae_scaling_factor)

        # Prepare pose condition image
        pose_cond_tensor = self.image_processor.preprocess(
            pose_image, height=height, width=width
        )
        pose_cond_tensor = pose_cond_tensor.to(
            dtype=self.vae.dtype, device=self.vae.device
        )
        pose_image_latents = self.vae.encode(pose_cond_tensor).latent_dist.sample().mul_(vae_scaling_factor)

        if use_fp16:
            pose_image_latents = pose_image_latents.half()
        pose_embedding = self.pose_guider(pose_image_latents)
        pose_embedding = (
            torch.cat([pose_embedding] * 2) if do_classifier_free_guidance else pose_embedding
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i == 0:
                    ref_latent_model_input = ref_image_latents.repeat(
                            (2 if do_classifier_free_guidance else 1), 1, 1, 1
                        )
                    reference_t = torch.zeros_like(t)
                    reference_t_expand = torch.tensor([reference_t] * ref_latent_model_input.shape[0], device=ref_latent_model_input.device)
                    if use_fp16:
                        ref_latent_model_input = ref_latent_model_input.half()
                        reference_t_expand = reference_t_expand.half()
